...
| `raw/tool-foreground-background-behavior.md` | 前台/后台工具机制观察（15 组实验），Bash/Agent/Workflow 前后台行为对比、task_started/task_notification 完整字段结构、task_type 映射（local_bash/local_agent/local_workflow）、run_in_background 参数传递规则、CLAUDE_CODE_DISABLE_BACKGROUND_TASKS 效果验证、**backgroundTasks() 前台转后台专项（case-12/13）：长前台 Bash 被 SDK 自动后台化、自动后台化不解除阻塞**、**streaming-input 模式专项（case-14）：streaming 续轮可行、interrupt() 对已收尾 turn 抛 "Query closed"**、**stopTask + 同步 interrupt 专项（case-15/16）：stopTask(taskId) 成功停任务+发 stopped 通知+query 存活可续轮、turn 活跃窗口内同步 interrupt() 成功打断阻塞、三方对照表**、**backgroundTasks 时机验证（case-17，解开 case-13→17 谜团）：控制方法必须在 task_started 之后调用才生效——case-14 早 4.2s 返回 false、case-17 等 task_started 后返回 true 且成功转后台（tool_result 带 backgroundTaskId、turn 立即继续）；真因是事件时序依赖，与输入模式/自动后台化/ID 匹配无关**、**前台/后台输出实时性专项（case-18，慢速多行命令）：前台任务输出【不能】实时拿到（stdout 跑完后一次性随 tool_result 返回、执行期事件流空白、gap≈命令时长）；后台任务输出【能】实时拿到（.output 文件行数随时间阶梯增长、可 tail）；官方 streaming 只流式 LLM 文本+工具入参、不流式工具 stdout**、**前台命令 .output 文件通道（case-19，解释 TUI Ctrl+O）：长前台命令被自动后台化后 .output 文件实时增长（逐秒 2→10 行、可用 session_id+task_id 拼路径 tail）；TUI Ctrl+O(transcript viewer) 实时输出来自该文件通道而非 SDK 事件流（强推断，未直接测 TUI）；调和 case-18a「前台不可实时」的边界=命令长度是否触发自动后台化** |
